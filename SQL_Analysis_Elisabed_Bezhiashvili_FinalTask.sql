...
WITH main AS (
    SELECT
        ...
        SUM(sale.quantity_sold) OVER (PARTITION BY chann.channel_id, country.country_region) AS sales,
		-- Calculate total sales per channel (all regions combined) to later compute sales %
        SUM(sale.quantity_sold) OVER (PARTITION BY chann.channel_id) AS total_sales_per_channel
    FROM sh.sales AS sale
...
ranked AS (
    SELECT 
        ...
        ROW_NUMBER() OVER (PARTITION BY channel_desc ORDER BY sales DESC) AS ranking
    FROM main
)
SELECT
    channel_desc,
...
WITH yearly_sales AS (
  ...
  GROUP BY products.prod_subcategory, times.calendar_year
),
with_lag AS (
  SELECT
    prod_subcategory,
...
growth_years AS (
  ...
  WHERE total_sales > prev_year_sales
    AND calendar_year BETWEEN 1999 AND 2001
),
...
WITH base_sales AS (
  ...
  WHERE 
    times.calendar_year IN (1999, 2000)
    AND channels.channel_desc IN ('Partners', 'Internet')
    AND products.prod_category IN ('Electronics', 'Hardware', 'Software/Other')
  GROUP BY
    times.calendar_year,
    times.calendar_quarter_desc,
    products.prod_category
...
q1_sales AS (
  SELECT
    ...
    sales AS q1_sales
  FROM base_sales
  WHERE calendar_quarter_desc LIKE '%-01'  -- only first quarter
),
...
final_report AS (
  ...
  FROM base_sales AS base
  LEFT JOIN q1_sales AS q1
    ON base.calendar_year = q1.calendar_year
   AND base.prod_category = q1.prod_category
  LEFT JOIN cumulative_sales_with_window AS cumulative
    ON base.calendar_year = cumulative.calendar_year
   AND base.calendar_quarter_desc = cumulative.calendar_quarter_desc
)
